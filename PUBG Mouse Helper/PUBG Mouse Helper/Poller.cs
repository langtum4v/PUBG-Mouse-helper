...
namespace PUBG_Mouse_Helper
{
    public class Poller
    {
        ...
        public static bool Auto_Tap = false;
        ...
        public void Poll(int dy, int shotInterval, int pullDelay, string fireButton, string tap)
        {
            this._dy = dy;
        ...
        private void PollFireButton()
        {
            ...
            Keys tap = Keys.XButton1;
            tap = HelperFunctions.GetAutoTapFromString(this._tap);
            short taps = GetAsyncKeyState(tap);
            if (Auto_Tap && ((taps & 0b10000000_00000000) > 0))
            {
                MouseHelperClass.LeftClickDown();
                MouseHelperClass.LeftClickUp();
            }

            Keys fireKey = Keys.XButton1; //default is middle mouse button
            fireKey = HelperFunctions.GetFireKeyFromString(this._fireButton);
            short gaks = GetAsyncKeyState(fireKey);
            if ((gaks & 0b10000000_00000000) > 0 || (Auto_Tap && ((taps & 0b10000000_00000000) > 0))) //if the key is set (non-zero) i.e. the key is being held down
            {
        ...
        private void PullDownMouse()
        {
            ...
            Keys tap = HelperFunctions.GetAutoTapFromString(this._tap);
            short taps = GetAsyncKeyState(tap);
            bool TapKeyPressed = (taps & 0b10000000_00000000) > 0;

            while (this.Activated && this.PerformRecoilCompensation && (fireKeyPressed || TapKeyPressed))
            {
                ...
                tap = HelperFunctions.GetAutoTapFromString(this._tap);
                taps = GetAsyncKeyState(tap);
                TapKeyPressed = (taps & 0b10000000_00000000) > 0;
            }
